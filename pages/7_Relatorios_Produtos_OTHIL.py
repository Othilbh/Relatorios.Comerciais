        ...
        if not texto_pdf:
            ...
            if not parsed['itens']:
                ...
                if st.button('💾 Salvar este Resumo do Estoque', key='prod_salvar_resumo_estoque'):
                    ...
                    # nunca fica numa tela de Dashboard depois de salvar.
                    acesso.redirecionar_pos_upload()

# Só a seção de upload acima é mostrada aqui -- os filtros, KPIs e as 12
# abas de relatório/Dashboard abaixo ficam disponíveis só na Gerência.
...
if dest:
    # Expander (26/08/2026, melhoria de navegação pedida pela Ingrid): reduz o
    # quanto de scroll é preciso antes da barra de abas aparecer, sem tirar
    # nenhuma informação dos cards -- fica expandido por padrão pra continuar
    # visível de cara.
    with st.expander('⭐ Destaques do Período', expanded=True):
        ...
        # Segunda fileira com as mesmas 4 colunas da primeira (a última fica
        # vazia) pra manter a largura dos cards consistente entre as duas
        # fileiras, em vez do 4+3 desalinhado de antes.
        d5, d6, d7, _d8_vazia = st.columns(4)
...
grp_visao, grp_analises, grp_cruzamentos, grp_historico = st.tabs([
    '📊 Visão Geral', '📈 Análises', '🔀 Cruzamentos', '🕓 Histórico e Alertas',
])
...
with grp_visao:
    tab_resumo, tab_margem, tab_drill, tab_trabalhados = st.tabs([
        '📋 Resumo por Produto', '💹 Margem de Contribuição', '🔍 Drill-down por Grupo', '🛠️ Mais Trabalhados',
    ])
...
with grp_analises:
    tab_top, tab_evo, tab_part = st.tabs([
...
with grp_cruzamentos:
    tab_pxv, tab_pxc, tab_matriz = st.tabs([
